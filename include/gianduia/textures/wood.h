...
namespace gnd {
    ...
    public:
        explicit WoodTexture(const PropertyList& props) {
            if constexpr (std::same_as<T, float>) {
                m_value1 = props.getFloat("value1", 0.0f);
                m_value2 = props.getFloat("value2", 1.0f);
            } else {
                m_value1 = props.getColor("value1", Color3f(0.6f, 0.4f, 0.2f)); 
                m_value2 = props.getColor("value2", Color3f(0.3f, 0.15f, 0.05f));
            }
            ...
            m_turbScale = props.getFloat("turb_scale", 2.0f);
        ...
        T evaluate(const SurfaceInteraction& isect) const override {
            ...
            float radius = Cross(Vector3f(isect.p), m_axis).length();

            radius += turb * m_turbScale;
            float t = std::abs(std::sin(radius * m_veinFreq));
